...
insert into Cliente (DNI, nombre, apellido, edad, direccion, email, telefono, cuentas) values ('34-3480976', 'Conant', 'Lidgard', 24, '34 Lakewood Point', 'clidgard0@ox.ac.uk', '4786467588', array_cuentas());
...
insert into Operacion VALUES (RetiradaUdt(3, 'DE33 7520 1499 2492 9685 27', TO_DATE( '2003-05-06' , 'YYYY-MM-DD'), '6:40 PM', 100.99, 'Para un capricho', (select ref(cu) FROM Cuenta cu WHERE cu.IBAN='DE33 7520 1499 2492 9685 27'), (select ref(ofi) FROM Oficina ofi WHERE ofi.codigo=795)));


insert into Operacion VALUES (RetiradaUdt(4, 'FR77 2620 0309 16V3 KGNX B3PC N29', TO_DATE( '2005-06-08' , 'YYYY-MM-DD'), '8:30 PM', 250.49, null, (select ref(cu) FROM Cuenta cu WHERE cu.IBAN='FR77 2620 0309 16V3 KGNX B3PC N29'), (select ref(ofi) FROM Oficina ofi WHERE ofi.codigo=795)));

-- TRANSFERENCIA

insert into Operacion VALUES (TransferenciaUdt(5, 'DE33 7520 1499 2492 9685 27', TO_DATE( '2001-01-01' , 'YYYY-MM-DD'), '5:30 PM', 500.00, null, (select ref(cu) FROM Cuenta cu WHERE cu.IBAN='FR77 2620 0309 16V3 KGNX B3PC N29'), (select ref(cu) FROM Cuenta cu WHERE cu.IBAN='DE33 7520 1499 2492 9685 27')));


insert into Operacion VALUES (TransferenciaUdt(6, 'MD72 OY6E HSY7 0HZK AM0D PAB0', TO_DATE( '2001-01-02' , 'YYYY-MM-DD'), '4:30 PM', 550.00, null, (select ref(cu) FROM Cuenta cu WHERE cu.IBAN='FR77 2620 0309 16V3 KGNX B3PC N29'), (select ref(cu) FROM Cuenta cu WHERE cu.IBAN='MD72 OY6E HSY7 0HZK AM0D PAB0')));
